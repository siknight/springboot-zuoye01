...
a.controller("navpage_controller",function ($scope,$http) {
      ...
      $scope.fun1=function () {
          ...
          f.success(function (result){
                if (result.code==1){
                    ...
                    $scope.navNum= result.data.studentNum;
                    console.log("navNum="+ $scope.studentNum);
                }
          });
    }
    ...
    //编辑方法
    $scope.nav_edit=function (title,url,id,name,age,sex,adress,w,h) {
        window.localStorage.setItem("student_edit_id",id);
        window.localStorage.setItem("student_edit_Name",name);
        window.localStorage.setItem("student_edit_age",age);
        window.localStorage.setItem("student_edit_sex",sex);
        window.localStorage.setItem("student_edit_adress",adress);
